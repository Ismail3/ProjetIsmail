...
class ProfilControleur extends AbstractControleur
{
...
<nav class="w3-sidebar w3-bar-block w3-black w3-card w3-animate-left w3-hide-medium w3-hide-large" style="display:none"
    ...
    public function displayUilisateurProfil()
    {
        ...
        $typePersonne = $_GET["typePersonne"];
        if (strcmp($typePersonne, Eleve::$TABLE_NAME) == 0) {
            $widgets = $widgets . $this->displayEleveProfil($idPersonne);
        } else {
            $widgets = $widgets . $this->displayEnseignantProfil($idPersonne);
        }
    ...
    function afficherEleve($id, $nom, $prenom, $email, $date_naissance, $niveau_etude, $image)
    {
        $eleve = '<div class="w3-col l3 m6 w3-margin-bottom">
            <div class="w3-card">
                <img src="' . $this->getImagePath() . $image . '" alt="' . $nom . ' ' . $prenom . '" style="width:100%">
...
//        Liste des cours crées
    ...
    private function displayEleveProfil($id)
    {
        $eleve = Eleve::getUtilisateur($id);
    ...
    private function displayEnseignantProfil($id)
    {
        $enseignant = Enseignant::getUtilisateur($id);
        $widgets = '<!-- Header with full-height image -->
            <div class="w3-content w3-margin-top" style="max-width:1400px;">
    ...
    private function getProfilMainInfos($personne, $type_personne)
    {
        ...
        $widget = $widget . '
                    <img src="' . $this->getImagePath() . $personne->getImage() . '" style="width:100%" alt="Avatar">';
        $widget = $widget . '
